 ...
 class User extends Database{
   ...
   public function all(){
    ...
    public function register($data){
        try{
            $result = parent::connect()->prepare("INSERT INTO users (email, name, password, img, id_rol) VALUES (?,?,?,?,?)");
            $result->bindParam(1, $data['email'], PDO::PARAM_STR);
            $result->bindParam(2, $data['name'], PDO::PARAM_STR);
            $result->bindParam(3, $data['password'], PDO::PARAM_STR);
            $result->bindParam(4, $data['img'], PDO::PARAM_STR);
            $result->bindParam(5, $data['id_rol'], PDO::PARAM_STR);
